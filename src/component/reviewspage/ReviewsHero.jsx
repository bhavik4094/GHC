...
function ReviewsHero() {
    ...
    useEffect(() => {
        const fetchHeroData = async () => {
            try {
                const response = await axios.get('http://localhost:5000/api/reviewhero');
                if (response.data) {
                ...
                setError('Failed to load Reviews Hero section.');
            } finally {
                setLoading(false);
            }
        };
    ...
    const starSvg = (
        <svg xmlns="http://www.w3.org/2000/svg" className="reviews-hero-stars" viewBox="0 0 24 24" fill="none">
            <g clipPath="url(#clip0_5552_28268)">
        ...
        <>
            {loading ? (
                ...
                <section className="video-banner-section">
                    ...
                    {/* footerPageTear image */}
                    <img
                        src={footerPageTear}
                        alt="Decorative page separator"
